...
function seedAll() {
  // Remove this line if already present
  ...
  // ✅ Always use crops.js for data
  let crops = window.defaultCrops || [];
  if (!crops.length) {
    console.warn("No crops found from crops.js");
  }

  // ✅ Reset crops if empty in localStorage
  if (!localStorage.getItem(CROPS_KEY) || read(CROPS_KEY).length === 0) {
    write(CROPS_KEY, crops);
    console.log("Loaded crops from crops.js:", crops);
  }
